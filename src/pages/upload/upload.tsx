...
function UploadPage() {
  ...
  const upload = async () => {
    try {
      ...
      //render chunks
      setChunks((preChunks) => {
        chunkArr = preChunks.map(({ fileHash, chunk, hash, percent }) => ({
          fileHash,
          chunk,
          hash,
          percent: uploadedChunks.includes(hash) ? 500 : percent,
        }));
        return chunkArr;
      });
  ...
  const createProgressHandler = (hash) => {
    ...
    return (e) => {
      setChunks((preChunks) => {
        ...
        preChunk.percent =
          initialPercent + (e.loaded / e.total) * (500 - initialPercent);
        return [...preChunks];
      });
    };
  };
      ...
      <Row key={chunk.hash}>
        ...
        <Col span={10} className="center">
          <Progress
            ...
            format={showStatus}
            status={
              uploadState === UPLOAD_STATES.FAILED && chunk.percent < 500
                ? "exception"
                : ""
            }
            strokeColor={
      ...
      <div className="container">
        <Row gutter={[16, 16]} justify="space-between">
          <Col>
            ...
            >
              <Button disabled={disableSelectFile} icon={<UploadOutlined />}>
                选择文件
              </Button>
            </Upload>
            {file && (
