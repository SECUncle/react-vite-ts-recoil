...
const getServeice = <T>(url: string, params?: any) => {
  const service = async () => {
    const transformRequest: AxiosTransformer = (data, headers) => {};
    // console.log('params: ', params);
    const data: T = await axios.get(`${url}`, {
...
const postService = <T>(url: string, params?: any) => {
  const service = async () => {
    const data: T = await axios.post(`${url}`, params);
    return data;
  };
  return service();
};

const putService = <T>(url: string, params?: any) => {
  const service = async () => {
...
const useAsyncCreate = <T>(url: string) => {
  const axios = useAxios();
  const queryClient = useQueryClient();
  return useMutation(async (params: T) => {
    const data: T = await axios.get(`${url}`, params);
